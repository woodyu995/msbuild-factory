...
| VS install exit **5003** / `Certificate is invalid: …\vs_installer.opc` | Windows | layout `certificates\`만으로는 부족. **Microsoft Windows Code Signing PCA 2024.crt** 필요(layout에 없음). 최신 `scripts/certs/` + install 스크립트가 자동 import. 로그에 `SCRIPT_REV=pca2024-certs-…` / `Code Signing PCA 2024` 가 보여야 함. Docker `COPY scripts` **Using cache**면 `--no-cache` 또는 `$WORK\scripts` 삭제 후 재빌드 |
